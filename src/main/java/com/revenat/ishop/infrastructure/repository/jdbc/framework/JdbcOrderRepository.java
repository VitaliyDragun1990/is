...
public class JdbcOrderRepository extends AbstractJdbcRepository implements OrderRepository {
	...
	public Order save(Order order) {
		...
		Order saved = executeUpdate(conn ->
			JDBCUtils.insert(conn, INSERT_ORDER, ORDER_HANDLER, order.getAccountId(), order.getCreated())
		);
	...
	public Order getById(long id) {
		// Get order by id
		Order order = executeSelect(conn -> JDBCUtils.select(conn, GET_ORDER_BY_ID, ORDER_HANDLER, id));
		if (order != null) {
	...
	public List<Order> getByAccountId(int accountId, int offset, int limit) {
		...
		List<Order> orders = executeSelect(conn ->
			JDBCUtils.select(conn, GET_ORDERS_BY_ACCOUNT_ID, ORDERS_HANDLER, accountId, limit, offset)
		);
	...
	public int countByAccountId(int accountId) {
		return executeSelect(conn -> JDBCUtils.select(conn, COUNT_ORDERS_BY_ACCOUNT_ID, COUNT_HANDLER, accountId));
	}
