...
public class JdbcProductRepository extends AbstractJdbcRepository implements ProductRepository {
	...
	@Override
	public int countAll() {
		return executeSelect(conn -> JDBCUtils.select(conn, COUNT_ALL_PRODUCTS, COUNT_HANDLER));
	}
	
	@Override
	public int countByCategory(String categoryUrl) {
		return executeSelect(conn -> JDBCUtils.select(conn, COUNT_PRODUCTS_BY_CATEGORY, COUNT_HANDLER, categoryUrl));
	}
	...
	public Product getById(Integer id) {
		return executeSelect(conn -> JDBCUtils.select(conn, GET_PRODUCT_BY_ID, PRODUCT_HANDLER, id));
	}

	@Override
	public List<Product> getByCategory(String categoryUrl, int offset, int limit) {
		return executeSelect(
				conn -> JDBCUtils.select(conn, GET_PRODUCTS_BY_CATEGORY, PRODUCTS_HANDLER, categoryUrl, limit, offset)
				);
	}
	...
	public List<Product> getAll(int offset, int limit) {
		return executeSelect(conn -> JDBCUtils.select(conn, GET_ALL_PRODUCTS, PRODUCTS_HANDLER, limit, offset));
	}
}
