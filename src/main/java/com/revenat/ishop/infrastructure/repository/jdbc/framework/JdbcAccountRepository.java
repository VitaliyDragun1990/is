...
public class JdbcAccountRepository extends AbstractJdbcRepository implements AccountRepository {
	...
	public Account getByEmail(String email) {
		return executeSelect(conn -> JDBCUtils.select(conn, GET_ACCOUNT_BY_EMAIL, ACCOUNT_HANDLER, email));
	}
	
	@Override
	public Account save(Account account) {
		Account saved = executeUpdate(conn -> 
			JDBCUtils.insert(conn, INSERT_ACCOUNT, ACCOUNT_HANDLER, account.getName(), account.getEmail(), account.getAvatarUrl())
		);
