...
public class JdbcOrderItemRepository extends AbstractJdbcRepository implements OrderItemRepository {
	...
	public OrderItem save(OrderItem orderItem) {
		OrderItem saved = executeUpdate(conn -> JDBCUtils.insert(conn, INSERT_ORDER_ITEM, ORDER_ITEM_HANDLER,
	...
	public void saveAll(Iterable<OrderItem> orderItems) {
		...
		executeUpdate(conn -> JDBCUtils.insertBatch(conn, INSERT_ORDER_ITEM, parametersList));
	}
	...
	public List<OrderItem> getByOrderId(long orderId) {
		return executeSelect(conn -> JDBCUtils.select(conn, GET_ORDER_ITEMS_BY_ORDER_ID, ORDER_ITEMS_HANDLER, orderId));
	}
}
