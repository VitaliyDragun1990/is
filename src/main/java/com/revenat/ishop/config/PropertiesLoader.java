...
public class PropertiesLoader {
	...
	private void validateResourceName(String resourceName) {
		Objects.requireNonNull(resourceName, "Resource name to load properties from can not be null");
		if (resourceName.trim().length() == 0) {
			throw new IllegalArgumentException("Resource name to load properties from can not be empty");
		}
	}
}
