...
public class ShoppingCartUtils {
	...
	public static ShoppingCart getCurrentShoppingCart(HttpSession session) {
		ShoppingCart cart = (ShoppingCart) session.getAttribute(Constants.CURRENT_SHOPPING_CART);
		if (cart == null) {
	...
	public static boolean isCurrentShoppingCartExists(HttpSession session) {
		return session.getAttribute(Constants.CURRENT_SHOPPING_CART) != null;
	}
	...
	public static void removeCurrentShoppingCart(HttpSession session, HttpServletResponse resp) {
		session.removeAttribute(Constants.CURRENT_SHOPPING_CART);
		CookieUtils.setCookie(Constants.Cookie.SHOPPING_CART.getName(), null, 0, resp);
	}
	...
	public static Cookie findShoppingCartCookie(HttpServletRequest req) {
		return CookieUtils.findCookie(req, Constants.Cookie.SHOPPING_CART.getName());
	}
	...
	public static void setCurrentShoppingCartCookie(String cookieValue, HttpServletResponse resp) {
		CookieUtils.setCookie(Constants.Cookie.SHOPPING_CART.getName(), cookieValue,
